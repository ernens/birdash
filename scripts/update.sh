...
# package-lock.json is auto-generated by npm install and can be safely
# reset — it's the most common cause of blocked updates on remote stations.
if ! git diff --quiet || ! git diff --cached --quiet; then
    # Auto-reset package-lock.json if it's the only dirty file.
