...
- <img src="docs/icons/microscope.svg" width="16" align="top" alt=""> **Module de raffinement des détections** — bbox heuristique calculée à chaque détection (localisation temps-fréquence visualisée par un encadré amber sur tous les spectrogrammes du dashboard) + worker stability check optionnel (`birdengine-stability.service`, opt-in) qui re-lance Perch sur une fenêtre 5 s recentrée sur le pic d'énergie ; les détections dont la confiance s'effondre au recentrage sont marquées `unstable` et remontent dans la liste des flagged. Voir [`docs/refinement/SPEC-v2.md`](docs/refinement/SPEC-v2.md)
